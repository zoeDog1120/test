package com.moon_and_earth;
import static com.moon_and_earth.ShaderUtil.createProgram;
...
{	
    ...
    public void initVertexData(float r)
    {
    	...
    	final float angleSpan=10;//将球进行单位切分的角度
    	for(float vAngle=180;vAngle>0;vAngle=vAngle-angleSpan)//垂直方向angleSpan度一份
        {
        	for(float hAngle=360;hAngle>0;hAngle=hAngle-angleSpan)//水平方向angleSpan度一份
        	{//纵向横向各到一个角度后计算对应的此点在球面上的坐标    	
        		double xozLength=r*UNIT_SIZE*Math.sin(Math.toRadians(vAngle));
        		float x1=(float)(xozLength*Math.cos(Math.toRadians(hAngle)));
				float y1=(float)(r*UNIT_SIZE*Math.cos(Math.toRadians(vAngle)));
        		float z1=-(float)(xozLength*Math.sin(Math.toRadians(hAngle)));
        		
        		xozLength=r*UNIT_SIZE*Math.sin(Math.toRadians(vAngle-angleSpan));
        		...
        		xozLength=r*UNIT_SIZE*Math.sin(Math.toRadians(vAngle));
        		float x4=(float)(xozLength*Math.cos(Math.toRadians(hAngle-angleSpan)));
        		float y4=(float)(r*UNIT_SIZE*Math.cos(Math.toRadians(vAngle)));
