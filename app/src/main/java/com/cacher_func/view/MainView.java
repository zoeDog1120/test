...
import com.cacher_func.thread.Angle2DThread;
...
public class MainView extends BNAbstractView{
	...
	public void initView() 
	{
		...
		if(!isBGMusic){
			//创建音乐
			if(!musicOff){
				MyActivity.sound.playBackGroundMusic(mv.activity, R.raw.nogame);
			}
		}
		...
		if(isSet)//这是游戏设定按钮的标志位
		{
	    	...
	    	case MotionEvent.ACTION_UP:
						...
						y>StartGame_TOUCH_TOP_y&&y<StartGame_TOUCH_BOTTOM_y&&!isSet&&isStartGm)
	    		{
	    			...
	    			if(!isBGMusic){
		    			if(!musicOff){
							MyActivity.sound.playBackGroundMusic(mv.activity, R.raw.game);
		    			}
	    			}
	    	...
	    	case MotionEvent.ACTION_DOWN:
						...
						y>StartGame_TOUCH_TOP_y&&y<StartGame_TOUCH_BOTTOM_y&&!isSet)
	    		{
	    			isStartGm=true;
	    			if(!effictOff){
						MyActivity.sound.playMusic(SOUND_Click,0);
	    			}
	    		}else if(x>YXJX_TOUCH_LEFT_x&&x<YXJX_TOUCH_RIGHT_x&&
