...
// uniform vec3 textureSize; // 纹理大小（宽和高），为了计算周围各点的纹理坐标，必须传入它，因为纹理坐标范围是0~1  
...
int getIsStrokeWithAngel(float angel)
{
    ...
    vec3 textureSize = vec3(754.0,376.0,0.0);
    int stroke = 0;
    ...
    if (a >= 0.1)// 我把alpha值大于0.5都视为不透明，小于0.5都视为透明
    {
        stroke = 1;
    }
    return stroke;
...
{   
    ...
    vec3 outlineColor=vec3(249, 170, 33); // 描边颜色
